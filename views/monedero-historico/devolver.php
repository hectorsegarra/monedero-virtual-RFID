...
$this->title = \Yii::t('app', 'Crear devolucion');
    ...
    <div class="monedero-historico-form">
        ...
        <?php
            ...
            //se modifican
             ...
             echo $form->field($model, 'tipo_operacion')->hiddenInput(['value'=>'Devolución'])->label(false);
             echo $form->field($model, 'concepto')->hiddenInput(['value'=> 'Devolución correspondiente a la operación: '.$model->id])->label(false);
             echo $form->field($model, 'importe')->hiddenInput(['value'=> -1*$model->importe])->label(false);
         ?>
        <div class="row">
            <div class="col-md-4">
                <?= Html::submitButton(\Yii::t('app', 'Emitir devolución'), ['class' =>  'btn btn-success' ]) ?>
            </div>
        </div>
